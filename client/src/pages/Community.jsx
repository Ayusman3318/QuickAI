...
const Community = () => {
  ...
  const imageLikeToggle = async (id) => {
    try {
      ...
      if (data.success) {
        setcreations(prev =>
          prev.map(c =>
            c.id === id ? { ...c, likes: data.updatedLikes } : c
          )
        );
      } else {
  ...
  return !loading ? (
          ...
          creations.map((creation, index)=>(
            ...
            sm:max-w-1/2 lg:max-w-1/3'>
              ...
              <div className='flex gap-1 items-center'>
                ...
                <Heart onClick={()=> imageLikeToggle(creation.id)} className={`min-w-5 hover:scale-110 hover:shadow-2xl
    ...
    <div className='flex justify-center items-center h-full'>
      <span className="w-10 h-10 border-4 border-primary
